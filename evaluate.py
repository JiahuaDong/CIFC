...
def clipeval(image_dir, candidates_json, device):
    ...
    with open(candidates_json) as f:
        list = json.load(f)
    candidates = {}
    for prompt_dict in list:
        candidates.update(prompt_dict) 
...
def calmetrics(sample_root, target_paths, numgen, outpkl):
    ...
    for i, target_path in enumerate(target_paths.split('+')):
        ...
        if i > 0:
            ...
            sd['CLIP Image alignment'] = imagealignment.astype(float)
            sd['DINO Image alignment'] = dinoimagealignment.astype(float)
    ...
    print("Metrics:", full)
    basename = basename=os.path.splitext(os.path.basename(sample_root))[0]
    with open(os.path.join(sample_root, f'{basename}_results.json'), 'w') as fr:
        json.dump(sd, fr, indent=4)
...
def parse_args():
    ...
    parser.add_argument("--sample_root", type=str,default='./results/ours/<dog1> <dog2>',
                        help="the root folder to generated images")
    parser.add_argument("--numgen", type=int, default=500,
                        help="total number of images.")
    parser.add_argument("--target_paths", type=str, default='./images/dog',
