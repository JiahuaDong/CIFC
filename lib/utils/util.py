...
def set_path_logger(accelerator, output_dir, config_path, opt, is_train=True):
    opt['is_train'] = is_train

    if is_train:
        experiments_root = osp.join(output_dir, opt['name'])
        ...
        results_root = osp.join(output_dir, 'results', opt['name'])
...
class MessageLogger():
    ...
    def __call__(self, log_vars):
        ...
        message = (
            f'[{self.exp_name}..][Iter:{current_iter:5,d}, lr:('
        )
        for v in lrs:
            message += f'{v:.3e},'
        message += ')] '
