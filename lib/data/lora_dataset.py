...
from lib.data.pil_transform import PairCompose, build_transform
...
class LoraDataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        data_dir = opt.get('data_dir')
        caption_dir = opt.get('caption_dir')
        mask_dir = opt.get('mask_dir', None)
        ...
        for x in Path(data_dir).iterdir():
            if x.is_file() and x.name != '.DS_Store':
                ...
                if use_caption and caption_path is not None and os.path.exists(caption_path):
                    ...
                    instance_prompt_image = replace_mapping.keys()[0]
